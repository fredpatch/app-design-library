...
export function StatsGrid({ items, loading = false, columns = 4, loadingLabel = 'Chargement des indicateurs' }: StatsGridProps) {
  if (loading) {
    return (
        ...
        const content = (
          <>
            <div className="fp-stat-card__header">
              ...
              {item.trend ? <StatusBadge tone={item.tone ?? 'neutral'} size="sm">{item.trend}</StatusBadge> : null}
            </div>
